---
name: endo-transcribe-v8
description: |
  Digitize handwritten Greek endocrinologist notes into structured Word documents (.docx) using the v8 format (3-phase: transcribe, medical review, assemble). Use this skill when the user explicitly asks for "v8" transcription or says "transcribe Patient001 v8", "digitize patient 002 with review", "v8 for Patient003". V8 adds an AI medical review pass that catches transcription errors, medical inconsistencies, and known OCR mistakes before generating the final document. For standard v7 transcription without the review pass, use the endo-transcribe skill instead.
---

# Endocrinologist Note Transcription — v8 Format (3-Phase)
...
**V8 vs V7**: The docx output format is identical. V8 adds Phase 2 — a medical review pass that catches transcription errors, medical inconsistencies, and known OCR mistakes before code generation. This produces cleaner output with fewer red `[uncertain]` items and fewer hallucinated entries.

## Invocation

The user gives a patient ID like `Patient001` and specifies v8. The job:
...
```
${CLAUDE_PLUGIN_ROOT}/skills/endo-transcribe-v8/references/v8_template_generator.js
```
...
Convert the reviewed notes into the template's code format using the same helper functions as v7:
...
### Technical
- **Do NOT write docx code from scratch.** Copy `v8_template_generator.js` and adapt it.
- Image section MUST be a separate docx section (different margins: 0.5cm vs 2cm)
- Always use `fitToPage(imgW, imgH)` for image sizing — scales to fit within 750×1070 pixels
...
2. **Token estimation**: V8 uses more tokens than v7 due to Phase 2.
...
Examine existing v7 outputs for reference on formatting (v8 produces identical docx format):
